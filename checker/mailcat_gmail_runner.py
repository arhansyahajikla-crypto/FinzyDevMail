    ...
    async def check(email):
        ...
        try:
            ...
            if found and found.get('Google'):
                return {'email': email, 'status': 'EXISTS', 'provider': 'Google', 'matches': found['Google']}
            if err:
                return {'email': email, 'status': 'UNKNOWN', 'reason': str(err)}
            return {'email': email, 'status': 'NOT_FOUND', 'reason': 'Mailcat tidak menerima alamat pada tahap RCPT TO.'}
