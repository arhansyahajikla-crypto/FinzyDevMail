...
function auth(req,res,next){const h=req.headers.authorization||"";if(!h.startsWith("Bearer "))return res.status(401).json({error:"Unauthorized"});try{req.user=jwt.verify(h.slice(7),SECRET);next()}catch{return res.status(401).json({error:"Session expired"})}}
...
app.post("/api/submissions",auth,(req,res)=>{
  ...
  const id=db.transaction(()=>{const s=db.prepare("INSERT INTO submissions(user_id,total_items,status,accepted_items,rejected_items) VALUES(?,?,?,0,0)").run(req.user.id,a.length,"PENDING");const ins=db.prepare("INSERT INTO submission_items(submission_id,email,normalized_email,status) VALUES(?,?,?,?)");for(const e of a)ins.run(s.lastInsertRowid,e,e,"PENDING");audit(req.user.id,"SUBMISSION_CREATED","submission",s.lastInsertRowid,{count:a.length});return s.lastInsertRowid})();
...
bootstrapAdmin().then(()=>app.listen(process.env.PORT||3000,()=>console.log(`FinzyDevMail ${VERSION} running`))).catch(e=>{console.error(e);process.exit(1)});
