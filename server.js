  ...
  db.transaction(()=>{
    ...
    if(st==="DITERIMA"){
      ...
      if(Number(s.credit_applied||0)!==1){
        ...
        db.prepare("INSERT INTO transactions(user_id,type,amount,status,note) VALUES(?,?,?,?,?)").run(s.user_id,"SETORAN",amount,"SUCCESS",`Setoran #${s.id} • ${s.total_items} email × Rp${Number(s.price_per_email||getPrice()).toLocaleString("id-ID")}`);
        db.prepare("UPDATE submissions SET credit_applied=1 WHERE id=?").run(s.id);
      }
    }
    audit(req.user.id,"STATUS_CHANGED","submission",s.id,{from:s.status,to:st,credited:st==="DITERIMA"?Number(s.total_items)*getPrice():0});
  })();
  res.json({ok:true,status:st,credited:st==="DITERIMA"?Number(s.total_items)*getPrice():0});
});
